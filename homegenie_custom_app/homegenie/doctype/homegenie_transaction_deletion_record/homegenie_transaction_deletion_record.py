...
class HomegenieTransactionDeletionRecord(Document):
	...
	def before_submit(self):
		if queued_docs := frappe.db.get_all(
			"Homegenie Transaction Deletion Record",
			...
			frappe.throw(
				_(
					...
					comma_and([get_link_to_form("Homegenie Transaction Deletion Record", x) for x in queued_docs]),
		...
		if self.status != "Running":
			frappe.throw(
				_("{0} is not running. Cannot trigger events for this Document").format(
					get_link_to_form("Homegenie Transaction Deletion Record", self.name)
				)
			)
		...
		if running_deletion_jobs := frappe.db.get_all(
			...
			frappe.throw(
				...
				msg=_("Transaction Deletion Document: {0} is running for this Company. {1}").format(
					get_link_to_form("Homegenie Transaction Deletion Record", running_deletion_jobs[0].name), err_msg
				),
			)
